...
async function initializeBarangays() {
    try {
        ...
        console.error("Failed to initialize barangays:", e);
    }
}
...
class MapTrends {
    constructor(e, a, t, o) {
        this.season = e.charAt(0).toUpperCase() + e.slice(1).toLowerCase();
        this.type = a;
        this.crop = t.charAt(0).toUpperCase() + t.slice(1);
        this.category = o;
    }
    generateMapTrends(e, t, o, a) {
        $("title").empty();
        $("#title").html(`<p>${a}</p>`);
        $(".label-box").empty();
        e.forEach((a) => {
            if (!t.find((e) => e.barangayName === a.barangayName)) {
                t.push({
        ...
        const n = t.map((e) => e[o]);
        const r = n.reduce((e, a) => e + a, 0) / n.length;
        const i = Math.sqrt(
            n.map((e) => Math.pow(e - r, 2)).reduce((e, a) => e + a, 0) /
                n.length
        );
        t.forEach((e) => {
            e.zScore = (e[o] - r) / i;
        ...
        function s(e) {
            if (e > 1.5) return "#008000";
            if (e >= 0.5) return "#FFA500";
            return "#FF0000";
        }
        return { barangays: e, data: t, getColor: s };
    }
    displayMapTrends(e, a, i, t, s) {
        let l = L.layerGroup().addTo(globalMap);
        globalMap.eachLayer((e) => {
            if (e instanceof L.CircleMarker && e.isPopupOpen()) {
                e.closePopup();
            }
        });
        const {
            ...
            data: c,
            getColor: d,
        } = this.generateMapTrends(e, a, i, t);
        setTimeout(() => {
            l.clearLayers();
            o.forEach((a) => {
                const { lat: e, lon: t } = getLatLon(a.coordinates);
                const o = c.find((e) => e.barangay === a.barangayName);
                if (o) {
                    const r = d(o.zScore);
                    var n = L.circleMarker([e, t], {
                        radius: 8,
                        color: "black",
                        fillColor: r,
                        fillOpacity: 1,
                        ...
                        .bindPopup(
                            `<strong>${a.barangayName}:</strong><br>${s}: ${o[i]}`
                        )
                        .on("popupopen", function () {
                            globalMap.panTo(n.getLatLng());
                        });
                    l.addLayer(n);
                    markers.push({
                        lat: e,
                        lon: t,
                        color: r,
                        popupText: `<strong>${a.barangayName}:</strong><br>${s}: ${o[i]}`,
                    });
                }
            });
            interpret(c, i, s);
...
function interpret(e, a, t) {
    ...
    const o = e[0].cropName;
    const n = aggregateProduction(e, a);
    const r = n.filter((e) => e.total !== 0);
    const i = n.filter((e) => e.total === 0);
    if (r.length === 0) {
        return `
        <div class="alert alert-warning" role="alert">
    ...
    const s = [...r].sort((e, a) => a.total - e.total);
    const l = s.map((e) => e.barangay);
    const c = s.map((e) => e.total);
    const d = s
        .map(
            (e, a) => `
...
`
    ...
    const p =
        i.length > 0
            ? `<p class="text-danger">Data not available for: ${i
                  .map((e) => e.barangay)
                  .join(", ")}.</p>`
            : "";
    const g = `
...
`;
    ...
    $("#interpretation").html(u);
    const h = document.getElementById("pieChart").getContext("2d");
    new Chart(h, {
        type: "pie",
        data: {
            labels: l,
            datasets: [
                {
                    label: `Total ${formatKey(a)}`,
                    data: c,
                    backgroundColor: generateColors(c.length),
                    borderColor: "#fff",
                    borderWidth: 2,
                },
            ],
...
function aggregateProduction(e, o) {
    const n = {};
    e.forEach((e) => {
        const a = e.barangay;
        const t = e[o] || 0;
        if (!n[a]) {
            n[a] = { barangay: a, total: 0 };
        }
        n[a].total += t;
    });
...
async function updateCropOptions() {
    ...
    const a = $("#season").val().toLowerCase();
    let t = "";
    try {
        if (e === "rice") {
            t = '<option value="rice">Rice</option>';
        } else {
            const n = await getUniqueCropNames(a, e);
            t =
                n.length > 0
                    ? n
                          .map(
        ...
        console.error("Failed to update crop options:", o);
        t = '<option value="">Error loading crops</option>';
    }
    $("#crop").html(t);
}
async function handleCategoryChange() {
    ...
    let n,
        ...
        l,
        c;
    l = await getYearRange();
    downloadYR = l;
    switch (o) {
        case "area_planted":
            s = "areaPlanted";
            if (t === "rice") {
                i = await getRiceProduction(t, e);
            } else {
                i = await getProduction(t, e);
            }
            n = `Area Planted Per Barangay (${l})`;
            r = stats.countAverageAreaPlantedBarangay(i);
            c = "area planted";
            break;
        case "production_volume":
            s = "volumeProductionPerHectare";
            if (t === "rice") {
                i = await getRiceProduction(t, e);
            } else {
                i = await getProduction(t, e);
            }
            n = `Production Volume per Hectare Per Barangay (${l})`;
            r = stats.averageVolumeProductionBarangay(i);
            c = "production volume per hectare";
            break;
        case "pest_occurrence":
            s = "pestOccurrence";
            i = await getPest(t, e);
            n = `Pest Occurrence Per Barangay (${l})`;
            r = stats.countPestOccurrenceBarangay(i);
            c = "pest occurrence";
            break;
        case "disease_occurrence":
            s = "diseaseOccurrence";
            i = await getDisease(t, e);
            n = `Disease Occurrence Per Barangay (${l})`;
            r = stats.countDiseaseOccurrenceBarangay(i);
            c = "disease occurrence";
            break;
        case "price_income_per_hectare":
            s = "incomePerHectare";
            i = await getProduction(t, e);
            n = `Price Income per Hectare Per Barangay (${l})`;
            r = stats.priceIncomePerHectareBarangay(i);
            c = "price income per hectare";
            break;
        case "profit_per_hectare":
            s = "profitPerHectare";
            i = await getProduction(t, e);
            n = `Profit per Hectare Per Barangay (${l})`;
            r = stats.profitPerHectareBarangay(i);
            c = "profit per hectare";
            break;
        case "damages":
            n = `Damages Report (${l})`;
            s = "damagePercentage";
            i = await getDamages(t, e);
            r = stats.calculateDamagePerBarangay(i);
            c = "damage percentage";
            break;
        default:
            n = "Category not recognized";
    }
    if (r.length !== 0 && t !== null) {
        $("#unavailable").hide();
        $(".available").show();
        $("#interpretation").show();
        const g = new MapTrends(e, a, t, n);
        g.displayMapTrends(barangays, r, s, n, c);
        currentType = s;
    } else {
        $(".available").hide();
        $("#interpretation").hide();
    ...
    downloadData = r;
    const d = downloadData[0] ? Object.keys(downloadData[0]) : [];
    function p(a) {
        return d.every((e) => a.hasOwnProperty(e));
    }
    downloadData = downloadData.filter(p);
}
function populateCategoryOptions(e) {
    ...
    let a;
    if (e === "Rice") {
        a = ["production_volume", "area_planted"];
    } else {
        a = Object.keys(o);
    }
    a.forEach((e) => {
...
function getLatLon(e) {
    const [a, t] = e.split(",").map(parseFloat);
    return { lat: a, lon: t };
}
async function download(e, a, t) {
...
function formatWithPesoSign(e, a) {
    const t = [
        "incomePerHectare",
        "profitPerHectare",
        "price",
    ...
    const t = [
        ...
        ...e.map((o) =>
            a
                .map((e) => {
                    const a = o[e];
                    const t = formatWithPesoSign(e, a);
                    return t || n(a);
                })
                .join(",")
        ),
    ].join("\n");
    const o = `Map Trends Crops Data Barangay.csv`;
    const r = new Blob([t], { type: "text/csv" });
...
function downloadExcel(e) {
    ...
    const n = a.addWorksheet("Map Trends Crops Data");
    n.addRow(["Crops Data"]).font = { bold: true };
    const r = n.addRow(o);
    r.eachCell((e) => {
        e.style = t;
    });
    e.forEach((t) => {
        const e = o.map((e) => {
            const a = t[e];
            return formatWithPesoSign(e, a) || a;
        });
        const a = n.addRow(e);
        a.eachCell((e) => {
            e.border = {
    ...
    const i = `Map Trends Crops Data Barangay.xlsx`;
    a.xlsx.writeBuffer().then((e) => {
        const a = new Blob([e], { type: "application/octet-stream" });
...
function downloadPDF(e) {
    ...
    const a = window.open("/print-map-trends", "_blank");
    addDownload(e, "PDF");
}
